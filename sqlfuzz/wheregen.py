...
def select_word(src_data, type):
    ...
    if len(oneword) == 1:
        return oneword
    offset = sqlfuzz.randoms.random_int_range(len(oneword) - 1)
    length = sqlfuzz.random_int_range(len(oneword) - offset)
...
def select_number(src_stat, src_data, type):
    if type == "op":
        return sqlfuzz.randoms.random_int_range(100)
    elif type == "comp":
...
def where_generator(src_col, dst_col, src_stat, dst_stat, src_data):
    ...
    # print("choose column for predicate generation!",
    #   src_col, type(src_col), srctype)
    if src_data is None:
        # if src data is none, create a random data
        if "CHAR" in srctype:
            src_data = ["".join(random.choices(
                ascii_lowercase, k=random.randint(1, 10))) for _ in range(10)]
    ...
    if "CHAR" in srctype:
        ...
        if sqlfuzz.randoms.random_int_range(1000) < 900:
        ...
        if sqlfuzz.randoms.random_int_range(1000) < 100:
            # cast the result
        ...
        op_select = random.choice(OP)
        op_num = select_number(src_stat, src_data, type="op")
        if (sqlfuzz.randoms.random_int_range(1000) < sqlfuzz.conf.PROB_TABLE["extractyear"]):
            # print("extract date")
            ...
            if (sqlfuzz.randoms.random_int_range(1000) < sqlfuzz.conf.PROB_TABLE["extractmonth"]):
...
def _mutate_value(bind_param, TF=None):
    ...
    if "CHAR" in srctype:
        bind_param.value = sqlfuzz.randoms.ret_randomdata_by_type("String")
    elif "FLOAT" in srctype:
        bind_param.value = sqlfuzz.randoms.ret_randomdata_by_type("Float")
        if TF is not None:
            bind_param.value = random.choice(out)
    elif "INT" in srctype:
        bind_param.value = sqlfuzz.randoms.ret_randomdata_by_type("Integer")
        if TF is not None:
            bind_param.value = random.choice(out)
    elif "DATE" in srctype or "TIME" in srctype:
        bind_param.value = sqlfuzz.randoms.ret_randomdata_by_type("DateTime")
